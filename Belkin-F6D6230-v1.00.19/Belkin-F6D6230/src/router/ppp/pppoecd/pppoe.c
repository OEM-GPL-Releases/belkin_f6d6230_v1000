...
int setdevname_pppoe(const char *cp)
{
	...
	if( ret == 1 && the_channel != &pppoe_channel )
	{
		...
		// Ron add for user request IP ++
		//printf("pppoe.c line %d, pppoe_req_ip = %08x\n", __LINE__, pppoe_req_ip);
		if(pppoe_req_ip != 0)
		{
			//ipcp_wantoptions[0].ouraddr = pppoe_req_ip;
			//ipcp_allowoptions[0].ouraddr = pppoe_req_ip;
		}
		// Ron add for user request IP --
