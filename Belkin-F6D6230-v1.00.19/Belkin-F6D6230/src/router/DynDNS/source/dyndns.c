...
static RC_TYPE do_update_alias_table(DYN_DNS_CLIENT *p_self)
{
	...
	if( (fp = fopen("/tmp/ddns.log", "w")) != NULL) 
	{
                   fprintf(fp,"ddns_start\n");
	}
		...
		for (i = 0; i < p_self->alias_info.count; ++i)
		{
			...
			/*build dyndns transaction*/
			{
				...
				if (p_self->dbg.level > 2)
				{
					...
					fprintf(fp,"ddns_status:connecting\n");
				}
				...
				if (rc == RC_OK)
				{
					...
					if (update_ok)
					{
						...
						fprintf(fp,"ddns_status:update OK\n");
					...
					else
					{
						...
						fprintf(fp,"ddns_status:username/passwd error!\n");
					...
					{							
						...
						fprintf(fp,"ddns_status:unknown error!\n");
					}
				}
			}
	...
	do
	{
		...
		if (rc != RC_OK)
		{
			DBG_PRINTF((LOG_WARNING,"W: DYNDNS: Error '%s' (0x%x) when talking to IP server\n",
			 ...
			 fprintf(fp,"ddns_status:connect error\n");
...
{
	RC_TYPE rc = RC_OK;
	int iterations = 0,error_number=0;
		...
		if (!os_handler_installed)
		{
			...
			if (rc != RC_OK)
			{
				DBG_PRINTF((LOG_WARNING,"DYNDNS: Error '%s' (0x%x) installing OS signal handler\n",
					errorcode_get_name(rc), rc));
				    fprintf(fp,"ddns_status:other error\n");
				break;
			}
		...
		while(1)
		{
			...
			if (rc != RC_OK)
			{
			 ...
			 if (rc == RC_DYNDNS_RSP_NOTOK )
                { 
                    DBG_PRINTF((LOG_ERR,"E: The response of DYNDNS svr was an error!\n"));
					  fprintf(fp,"ddns_status:unknown error!\n");
			error_number++;		
                }	
                if (rc == RC_DYNDNS_RSP_NOTOK && error_number>=3)
                { 
                    DBG_PRINTF((LOG_ERR,"E: The response of DYNDNS svr was an error! Aborting.\n"));
			  fprintf(fp,"ddns_status:disabled\n");		
                    break;              			
                }
			}
