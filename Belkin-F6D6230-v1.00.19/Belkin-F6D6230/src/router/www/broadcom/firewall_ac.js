...
function MEntry(host, mac, status, rest, ref, sche)
{
	...
	this.urls = new Array(URL_MAX);
	for(var i=0;i<URL_MAX;i++) this.urls[i] = "";
	this.keywords = new Array(KEYWORD_MAX);
	for(var i=0;i<KEYWORD_MAX;i++) this.keywords[i] = "";
};
//==============================================================================
function MemberList()
{
...
function DoSaveToFlash()
{
	...
	buf += CreateAjaxInput("ac_landing_password", document.tF.ac_landing_password.value);
...
function PrepareData(i)
{
	...
	for(var j=0;j<URL_MAX;j++)
		buf += CreateAjaxInput("url_"+j, e.urls[j]);
	
	for(var j=0;j<KEYWORD_MAX;j++)
