...
int main(int argc,char* argv[])
{
	...
	if(!strcmp(s.username,""))
	{
		...
		printf("bpalogin: Invalid username, exit process!!\n");
		exit(1);
	}
	if(!strcmp(s.password,""))
	{
		critical("Password has not been set");
		printf("bpalogin: Invalid password, exit process!!\n");
		exit(1);
	}
	s.debug = debug;
	s.critical = critical;
...
void bpalogin_set_domain_name(char *domain_name)
{
	if ((domain_name) && (domain_name[0] != 0))
	{
		strcpy(s.authdomain,".");
		strcat(s.authdomain, domain_name);
	}
	else
	{
		strcpy(s.authdomain,DEFAULT_AUTHDOMAIN);
