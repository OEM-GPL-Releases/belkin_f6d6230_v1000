 ...
 *
 * $Id: daemon.c 8903 2009-08-12 14:40:32Z charles $
 */
...
{
#ifdef TR_DCTCS
    fprintf(stderr, "=> %s dir=%s file=%s %s(%d)\n", __FUNCTION__, dir, file,
	    __FILE__, __LINE__);
#endif
...
main( int argc, char ** argv )
{
...
#ifdef TR_DCTCS    
    // TODO: on wg8116f22-lf-ak project, libevent doesn't work in epoll mode.
    setenv("EVENT_NOEPOLL", "yes", 1);
    // TODO: disable preallocation.
    tr_bencDictAddInt( &settings, TR_PREFS_KEY_PREALLOCATION, 0 );
#endif    
    ...
    /* remove all torrents */
    {
	tr_torrent *tr_torrentNext( tr_session  * session, tr_torrent  * current ); // From torrent.h
	tr_torrent * tor = NULL;
	while( ( tor = tr_torrentNext( mySession, tor ) ) )
	    tr_torrentRemove( tor );;
    }
    /* disable session whitelist */
    tr_sessionSetRPCWhitelistEnabled( mySession, FALSE);
