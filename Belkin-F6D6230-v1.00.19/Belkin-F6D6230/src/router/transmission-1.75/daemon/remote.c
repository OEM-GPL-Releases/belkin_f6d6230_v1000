 ...
 *
 * $Id: remote.c 8982 2009-08-22 15:08:07Z charles $
 */
...
enum { TAG_SESSION, TAG_STATS, TAG_LIST, TAG_DETAILS, TAG_FILES, TAG_PEERS, TAG_BFMAP };
...
static tr_option opts[] =
{
    ...
    { 955, "global-seedtime",      "All torrents, unless overridden by a per-torrent setting, should seed until a specific time", "gst", 1, "seedtime" },
    { 956, "no-global-seedtime",   "All torrents, unless overridden by a per-torrent setting, should seed regardless of seedtime", "GST", 0, NULL },
    { 957, "bf-priority",          "Set the current torrent(s) bitfilter idx to 1", "bfp", 1, "index" },
    { 958, "no-bf-priority",       "Let the current torrent(s) reset the bitfilter map", "BFP", 0, NULL },
    { 959, "bf-map",               "Show the current torrent BF map", "bfm", 0, NULL },
    ...
    *id = '\0';
    ...
    while( ( c = tr_getopt( getUsage( ), argc, argv, opts, &optarg ) ) )
    {
        ...
        switch( c )
        {
		    ...
		    tr_bencDictAddStr( args, "filename", buf );
#else
            ...
            case 955:
                tr_bencDictAddStr( &top, "method", "session-set" );
                tr_bencDictAddInt( args, "seedTimeLimit", atoi(optarg) );
                tr_bencDictAddBool( args, "seedTimeLimited", TRUE );
                break;

            case 956:
                tr_bencDictAddStr( &top, "method", "session-set" );
                tr_bencDictAddBool( args, "seedTimeLimited", FALSE );
                break;

            case 957:
                tr_bencDictAddStr( &top, "method", "torrent-set" );
                tr_bencDictAddInt( args, "bitfieldPriority", (unsigned int)atol(optarg) );
                addIdArg( args, id );
                break;
		
            case 958:
                tr_bencDictAddStr( &top, "method", "torrent-set" );
                tr_bencDictAddBool( args, "bitfieldPriorityReset", TRUE );
                addIdArg( args, id );
                break;

            case 959:
                tr_bencDictAddStr( &top, "method", "torrent-get" );
                tr_bencDictAddInt( &top, "tag", TAG_BFMAP );
                addIdArg( args, id );
                n = TR_N_ELEMENTS( bfmap_keys );
                fields = tr_bencDictAddList( args, "fields", n );
                for( i = 0; i < n; ++i )
                    tr_bencListAddStr( fields, bfmap_keys[i] );
                break;
...
printSession( tr_benc * top )
{
    ...
    if( ( tr_bencDictFindDict( top, "arguments", &args ) ) )
    {
        ...
        int64_t      i;
        tr_bool      boolVal;
        int64_t seedTimeLimit;
        tr_bool seedTimeLimited;

        tr_bencDictFindInt ( args, "seedTimeLimit", &seedTimeLimit );
        tr_bencDictFindBool ( args, "seedTimeLimited", &seedTimeLimited );
        ...
        char bufSTL[64];
        etaToString(&bufSTL, sizeof(bufSTL), seedTimeLimit);
        printf( "  Global seed time limit: %s %s\n", (seedTimeLimited ? "Yes" : "No" ), bufSTL);
        time_t now = time(0);
        printf( "  Now: %s\n", ctime(&now) );
...
printDetails( tr_benc * top )
{
      ...
      && ( tr_bencDictFindList( args, "torrents", &torrents ) ) )
    {
             ...
             ++ti )
        {
            ...
            tr_bool      isStopped;
            tr_bool      boolVal;
            int64_t      seedTimeLimit, seedTimeMode, seedStartTime;

	        tr_bencDictFindInt( t, "seedTimeLimit", &seedTimeLimit);
	        tr_bencDictFindInt( t, "seedTimeMode", &seedTimeMode);
	        tr_bencDictFindInt( t, "seedStartTime", &seedStartTime);
            ...
            const char *seedTimeModeStr;
            switch ((int)seedTimeMode) {
            case TR_SEEDTIMELIMIT_GLOBAL: seedTimeModeStr = "Global"; break;
            case TR_SEEDTIMELIMIT_SINGLE: seedTimeModeStr = "Single"; break;
            default: seedTimeModeStr = "Unlimited";
            }
            char bufSTL[64];
            etaToString(&bufSTL, sizeof(bufSTL), seedTimeLimit);
            printf( "  Seed time mode: %s\n", seedTimeModeStr );
            printf( "  Seed start time: %s", (seedStartTime ? ctime((time_t *)&seedStartTime) : "0\n" ) );
            printf( "  Seed time limit: %s\n", bufSTL );
            time_t now = time(0);
...
printBfmap( tr_benc * top )
{
	...
	&& ( tr_bencDictFindList( args, "torrents", &torrents ) ) )
	{
		...
		if( tr_bencDictFindRaw( t, "pieces", &raw, &rawlen ) ) {
		int len;
		unsigned char * str = (unsigned char *)tr_base64_decode( raw, rawlen, &len );
		if (str) {
		for (i = 0; i < len; i++) {
		printf("%02x", str[i]);
		}
