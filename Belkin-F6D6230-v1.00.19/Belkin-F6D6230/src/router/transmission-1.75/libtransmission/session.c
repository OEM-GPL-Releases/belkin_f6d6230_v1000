 ...
 *
 * $Id: session.c 9074 2009-09-08 20:02:51Z charles $
 */
...
#ifdef TR_SEEDTIME
void
tr_sessionSetSeedTimeLimited( tr_session * session, tr_bool isLimited )
{
    assert( tr_isSession( session ) );

    session->isSeedTimeLimited = isLimited;
    updateSeedRatio( session );
}

void
tr_sessionSetSeedTimeLimit( tr_session * session, time_t seedTimeLimit )
{
    assert( tr_isSession( session ) );

    session->seedTimeLimit = seedTimeLimit;
    updateSeedRatio( session );
}

tr_bool
tr_sessionIsSeedTimeLimited( const tr_session  * session )
{
    assert( tr_isSession( session ) );

    return session->isSeedTimeLimited;
}

time_t
tr_sessionGetSeedTimeLimit( const tr_session * session )
{
    assert( tr_isSession( session ) );

    return session->seedTimeLimit;
}
#endif
