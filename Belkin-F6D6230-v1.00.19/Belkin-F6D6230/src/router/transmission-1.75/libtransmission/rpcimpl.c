 ...
 *
 * $Id: rpcimpl.c 8889 2009-08-10 20:04:08Z charles $
 */
...
#ifdef TR_DCTCS
int find_filename_pos_in_url(const char *s);
void save_upload_info(const char *arg_metainfo_file)
{
	FILE *fd1, *fd2;
	...
	filename_pos = find_filename_pos_in_url(arg_metainfo_file);
	if(filename_pos != -1)
	{
		strncpy(url1, arg_metainfo_file, 127);
		url1[filename_pos] = 0;
		strcat(url1, ".torrent_complete");

		strncpy(url2, arg_metainfo_file, 127);
		url2[filename_pos] = 0;
		strcat(url2, ".torrent_complete2");
		...
		if(fd1)
		{
			...
			if(fd2)
			{
				while(fscanf(fd1, "%s\t%lu\t%d\n", name, &sec, &upload) == 3)
				{
				    printf("%s name in file=%s, torrent name=%s\n", __FUNCTION__, name, arg_metainfo_file+filename_pos);
					if(strcmp(name, arg_metainfo_file+filename_pos) == 0)
					{
					    fprintf(fd2, "%s\t%lu\t1\n", arg_metainfo_file+filename_pos, time(&sec));
					} else {
					    fprintf(fd2, "%s\t%lu\t%d\n", name, sec, upload);
					}
				}
				fclose(fd2);
				sprintf(cmd, "mv \"%s\" \"%s\"", url2, url1);
				system(cmd);
			} else
			    printf("open %s fail\n", url2);
			fclose(fd1);
...
void save_download_info(const char *arg_metainfo_file)
{
	FILE *fd;
	char url[512];
	char filename_pos;
	...
	if(filename_pos != -1)
	{
		strncpy(url, arg_metainfo_file, 127);
		url[filename_pos] = 0;
		strcat(url, ".torrent_complete");
...
{
#ifdef DEBUG
    dbgmsg("=> %s torrent=0x%08x srcFilename=%s", __FUNCTION__, torrent, tr_torrentGetSrcFilename( torrent ));
#endif
    if (tr_torrentGetSrcFilename( torrent ))
	save_download_info(tr_torrentGetSrcFilename( torrent ));
}

static void
torrentRatioLimitHitChanged( tr_torrent       * torrent,
                            void             * user_data     UNUSED )
{
#ifdef DEBUG
    dbgmsg("=> %s torrent=0x%08x srcFilename=%s", __FUNCTION__, torrent, tr_torrentGetSrcFilename( torrent ));
#endif
    torrent->uploadedCur = 0;
    torrent->uploadedPrev = 0;
    if (tr_torrentGetSrcFilename( torrent )) {
	save_upload_info(tr_torrentGetSrcFilename( torrent ));
	// Try to stop torrent
	extern void removeUITorrent(int id) __attribute__((weak));
	extern void setTorrentComplete(int id) __attribute__((weak));

	const tr_stat *st = tr_torrentStat( torrent );
	removeUITorrent((int)st->id);
	setTorrentComplete((int)st->id);
    }
}
#endif
          ...
          tr_benc *          list )
{
    ...
    for( i = 0; i < info->fileCount; ++i )
    {
        const tr_file * file = &info->files[i];
#ifdef TR_DCTCS
	tr_benc *       d = tr_bencListAddDict( list, 5 );
#else	
        tr_benc *       d = tr_bencListAddDict( list, 3 );
#endif	
        tr_bencDictAddInt( d, "bytesCompleted", files[i].bytesCompleted );
...
#ifdef TR_DCTCS
        tr_bencDictAddInt( d, "firstPiece", file->firstPiece );
        tr_bencDictAddInt( d, "lastPiece", file->lastPiece );
#endif
    }
...
#ifdef TR_DCTCS
    else if( tr_streq( key, keylen, "srcFilename" ) )
        tr_bencDictAddStr( d, key, tr_torrentGetSrcFilename( tor ) );
#endif    
...
#ifdef TR_SEEDTIME
    else if( tr_streq( key, keylen, "seedTimeLimit" ) )
        tr_bencDictAddInt( d, key, tor->seedTimeLimit );
    else if( tr_streq( key, keylen, "seedTimeMode" ) )
        tr_bencDictAddInt( d, key, tr_torrentGetSeedTimeMode( tor ) );
    else if( tr_streq( key, keylen, "seedStartTime" ) )
        tr_bencDictAddInt( d, key, tor->seedStartTime );
#endif    
...
#ifdef TR_SEEDTIME
        if( tr_bencDictFindInt( args_in, "seedTimeLimit", &tmp ) )
            tr_torrentSetSeedTimeLimit( tor, tmp );
        if( tr_bencDictFindInt( args_in, "seedTimeMode", &tmp ) )
            tr_torrentSetSeedTimeMode( tor, tmp );
#endif
#ifdef TR_VUZE_BFP
        if( tr_bencDictFindInt( args_in, "bitfieldPriority", &tmp ) )
	    tr_torrentSetBitfieldPriority( tor, tmp );
	if( tr_bencDictFindBool( args_in, "bitfieldPriorityReset", &tmp ) )
	    if (tmp) tr_torrentResetBitfieldPriority( tor );
#endif	
        notify( session, TR_RPC_TORRENT_CHANGED, tor );
    }
...
addTorrentImpl( struct tr_rpc_idle_data * data, tr_ctor * ctor )
{
...
#ifdef TR_DCTCS
    char *filename = 0;
    if (tr_ctorGetSourceFile(ctor))
	filename = strdup(tr_ctorGetSourceFile(ctor));
#endif
    tr_ctorFree( ctor );
	...
	if (filename && *filename != 0) {
	    ...
	    tr_torrentSetRatioLimitHitCallback( tor, torrentRatioLimitHitChanged, NULL );
	    tr_torrentSetSrcFilename( tor, filename);
	    tr_free(filename);
	}
#endif	
            ...
            struct tr_rpc_idle_data  * idle_data )
{
...
#ifdef DEBUG
    dbgmsg("filename=%s %s(%d)", filename, __FILE__, __LINE__);
    dbgmsg("metainfo=%s %s(%d)", metainfo_base64, __FILE__, __LINE__);    
#endif
    ...
    else
    {
        ...
        {
#ifdef DEBUG
	    dbgmsg("CKPT %s(%d)", __FILE__, __LINE__);
#endif
            if( filename != NULL )
...
#ifdef TR_SEEDTIME
    if( tr_bencDictFindInt( args_in, "seedTimeLimit", &i ) )
        tr_sessionSetSeedTimeLimit( session, i );
    if( tr_bencDictFindBool( args_in, "seedTimeLimited", &boolVal ) )
        tr_sessionSetSeedTimeLimited( session, boolVal );
#endif    
...
#ifdef TR_SEEDTIME
    tr_bencDictAddInt ( d, "seedTimeLimit", tr_sessionGetSeedTimeLimit( s ) );
    tr_bencDictAddBool( d, "seedTimeLimited", tr_sessionIsSeedTimeLimited( s ) );
#endif
