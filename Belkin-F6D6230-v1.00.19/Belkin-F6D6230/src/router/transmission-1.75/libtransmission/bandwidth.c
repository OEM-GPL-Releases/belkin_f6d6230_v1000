 ...
 *
 * $Id: bandwidth.c 8483 2009-05-22 19:03:00Z charles $
 */
...
/* #define DEBUG_DELAY */
/* #define FIX_DELAY */
/* #define FIX_DELAY_MAX_INCREMENT 32768 */
/* #define FIX_DELAY_MAX_TIMES_THRESHOLD 2000 */
...
phaseOne( tr_ptrArray * peerArray, tr_direction dir )
{
...
#ifdef DEBUG_DELAY
    size_t pc = n;
    /* fprintf(stderr, "%s %d %s(%d)\n", __FUNCTION__, time(0), __FILE__, __LINE__); */
	...
	count++;
        ...
        int bytesUsed = tr_peerIoFlush( peers[i], dir, increment );
        ...
        if( i == n ) {
            i = 0;
	    if (count > FIX_DELAY_MAX_TIMES_THRESHOLD) {
		if (increment < FIX_DELAY_MAX_INCREMENT)
		    increment <<= 3;
	    }
	}
    }
    ...
    if (t > 0) {
#ifdef DEBUG
	fprintf(stderr, "%s peerCount(n)=%d\n", __FUNCTION__, pc);
	fprintf(stderr, "%s %d times:%u (%d secs) %s(%d)\n", __FUNCTION__, time(0), count, t, __FILE__, __LINE__);
#endif
    }
#endif
#else
...
#ifdef DEBUG_DELAY
#ifdef DEBUG
    fprintf(stderr, "%s %d loop:%u (%d secs) %s(%d)\n", __FUNCTION__, time(0), lll, time(0) - ttt, __FILE__, __LINE__);
#endif
#endif
#endif
}
