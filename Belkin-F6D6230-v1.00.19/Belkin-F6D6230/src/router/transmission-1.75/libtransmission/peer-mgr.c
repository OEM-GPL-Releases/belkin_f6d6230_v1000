 ...
 *
 * $Id: peer-mgr.c 9131 2009-09-17 03:24:35Z charles $
 */
...
#ifdef TR_VUZE_BFP
    if (a->priorityVuze != b->priorityVuze)
	return a->priorityVuze > b->priorityVuze ? -1 : 1;
#endif    
...
#ifdef TR_VUZE_BFP
	    setme->priorityVuze = inf->pieces[piece].priorityVuze;
#ifdef DEBUG
	    if (setme->priorityVuze)
		fprintf(stderr, "Find priorityVuze piece=%u\n", piece);
#endif
#endif	    
...
bandwidthPulse( void * vmgr )
{
    ...
    /* TODO: block timer event? */
    /* comment this can't download? */
    pumpAllPeers( mgr );
...
#ifdef TR_SEEDTIME
        if( tor->needsSeedTimeCheck ) {
            /* tor->needsSeedTimeCheck = FALSE; */
            tr_torrentCheckSeedTime( tor );
        }
#endif	
    }
