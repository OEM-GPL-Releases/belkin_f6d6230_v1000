 ...
 *
 * $Id: torrent.c 9041 2009-09-05 16:51:22Z charles $
 */
...
#ifdef TR_SEEDTIME
void
tr_torrentSetSeedTimeMode( tr_torrent *  tor, tr_seedtimelimit mode )
{
    assert( tr_isTorrent( tor ) );
    assert( mode==TR_SEEDTIMELIMIT_GLOBAL || mode==TR_SEEDTIMELIMIT_SINGLE || mode==TR_SEEDTIMELIMIT_UNLIMITED  );

    if( mode != tor->seedTimeLimitMode )
    {
        tor->seedTimeLimitMode = mode;
	tor->needsSeedTimeCheck = TRUE;
...
tr_seedtimelimit
tr_torrentGetSeedTimeMode( const tr_torrent * tor )
{
    assert( tr_isTorrent( tor ) );

    return tor->seedTimeLimitMode;
}

void
tr_torrentSetSeedTimeLimit( tr_torrent *tor, time_t seedTimeLimit ) {
    assert( tr_isTorrent( tor ) );

    tor->seedTimeLimit = seedTimeLimit;
}

tr_bool
tr_torrentGetSeedTimeLimit( const tr_torrent * tor, time_t * time )
{
    tr_bool isLimited;

    switch( tr_torrentGetSeedTimeMode( tor ) )
    {
        case TR_SEEDTIMELIMIT_SINGLE:
            isLimited = TRUE;
            if( time )
		*time = tor->seedTimeLimit;
            break;

        case TR_SEEDTIMELIMIT_GLOBAL:
	    isLimited = tor->session->isSeedTimeLimited;
            if( isLimited && time )
		*time = tor->session->seedTimeLimit;
            break;

        default: /* TR_SEEDTIMELIMIT_UNLIMITED */
            isLimited = FALSE;
            break;
    }
...
void
tr_torrentCheckSeedTime( tr_torrent * tor )
{
    time_t seedTimeLimit;

    assert( tr_isTorrent( tor ) );

#ifdef DEBUG
    fprintf(stderr, "=> %s %s\n", __FUNCTION__, tor->info.name);
#endif
    
    if( !tr_torrentIsSeed( tor ) )
	return;

    if( !tor->seedStartTime )
	tor->seedStartTime = time(0);

#ifdef DEBUG
    /* fprintf(stderr, "%s seedStartTime=%s", __FUNCTION__, ctime(&tor->seedStartTime)); */
#endif
    
    /* if we're seeding and we've reached our seed time limit, stop the torrent */
    if( tor->isRunning && tr_torrentIsSeed( tor ) && tr_torrentGetSeedTimeLimit( tor, &seedTimeLimit ) )
    {
        time_t now = time(0);
	time_t seedStartTime = tor->seedStartTime;

	#ifdef DEBUG
	printf("seedTimeLimit=%d/%d seconds seedStartTime=%s", (seedTimeLimit - (now - seedStartTime)),
	       seedTimeLimit, ctime(&seedStartTime) );
	#endif
        if( (now - seedStartTime) >= seedTimeLimit )
        {
            tr_torrentStop( tor );

            /* set to no ratio limit to allow easy restarting */
	    /* tr_torrentSetSeedTimeMode( tor, TR_SEEDTIMELIMIT_UNLIMITED ); */
	    tor->seedStartTime = 0;
...
#ifdef TR_VUZE_BFP
void          tr_torrentSetBitfieldPriority( tr_torrent     * tor,
					     uint32_t      bfidx)
{
#ifdef DEBUG
    fprintf(stderr, "==> %s bfidx=%u\n", __FUNCTION__, bfidx);
#endif
    assert( tr_isTorrent( tor ) );
    if (bfidx > tor->info.pieceCount) {
#ifdef DEBUG
	fprintf(stderr, "bfidx out of range. (should be 0~%u\n", tor->info.pieceCount - 1);
#endif
	return;
    }
    tor->info.pieces[bfidx].priorityVuze = 1;
#ifdef DEBUG
    tr_piece_index_t i;
    int e;
    i = 0;
    printf("  ");
    while ( i < tor->info.pieceCount ) {
	for( e=0; i<tor->info.pieceCount && e<8; ++e, ++i )
	    printf("%d", tor->info.pieces[i].priorityVuze);
	printf( " " );
	if( !(i%64) )
	    printf( "\n  " );
    }
...
void          tr_torrentResetBitfieldPriority( tr_torrent   * tor ) {
    tr_piece_index_t i;
    assert( tr_isTorrent( tor ) );
    for (i = 0; i < tor->info.pieceCount; i++) {
	tor->info.pieces[i].priorityVuze = 0;
    }
}
#endif
...
#ifdef TR_DCTCS
    tor->srcFilename = 0;
    /* fprintf(stderr, "CKPT %s(%d)\n", __FILE__, __LINE__); */
    if (tr_ctorGetSourceFile(ctor))
	tr_torrentSetSrcFilename( tor, tr_ctorGetSourceFile(ctor));
    /* fprintf(stderr, "CKPT %s(%d)\n", __FILE__, __LINE__); */
#endif    
...
#ifdef TR_DCTCS_VUZE
    doStart = 0;
#elif TR_DCTCS /* TODO: Auto start by transmission core. Could hang up the core! */
    doStart = 0;
#else
...
#if defined(TR_DCTCS) && defined(TR_SEEDTIME)
    tr_torrentSetRatioMode( tor, TR_RATIOLIMIT_GLOBAL );
    tr_torrentSetRatioLimit( tor, tr_sessionGetRatioLimit( tor->session ) );
    tr_torrentSetSeedTimeMode( tor, TR_SEEDTIMELIMIT_GLOBAL );
    tr_torrentSetSeedTimeLimit( tor, tr_sessionGetSeedTimeLimit( tor->session ) );
#else
...
#ifdef TR_DCTCS
void
tr_torrentSetSrcFilename( tr_torrent * tor, const char * fileName )
{
#ifdef DEBUG
    fprintf(stderr, "=> %s fileName=%s\n", __FUNCTION__, fileName);
#endif
    assert( tr_isTorrent( tor ) );
    if ( !fileName || !tor->srcFilename || strcmp( fileName, tor->srcFilename ) ) {
	    tr_free( tor->srcFilename );
	    tor->srcFilename = tr_strdup(fileName);
    }
}

const char *
tr_torrentGetSrcFilename( tr_torrent * tor)
{
    assert( tr_isTorrent( tor ) );
    
    return (tor->srcFilename);
}
#endif
...
/* 	    tor->seedStartTime = time(0); */
/* #ifdef DEBUG */
/* 	    fprintf(stderr, "%s Change tor->seedStartTime from %d to %d %s(%d)\n", __FUNCTION__, tor->seedStartTime, time(0), */
/* 		    __FILE__, __LINE__); */
/* #endif */
/* 	} */
#endif
