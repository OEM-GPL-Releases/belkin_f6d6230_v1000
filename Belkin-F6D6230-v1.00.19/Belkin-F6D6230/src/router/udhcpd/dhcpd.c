...
static void signal_handler(int sig)
{
	...
	if (sig == SIGUSR1)  {
		//cprintf("%s(%d): dhcpd signal handler\n", __FUNCTION__, __LINE__);
		read_leases(server_config.lease_file); // refresh the lease 
	}
}
	...
	while(1) { /* loop until universe collapses */
		...
		if (FD_ISSET(signal_pipe[0], &rfds)) {
			...
			case SIGUSR1:
				...
				//cprintf("%s(%d): Received a SIGUSR1\n", __FUNCTION__, __LINE__);
				write_leases();
			...
			case SIGTERM:
				...
				//cprintf("%s(%d): Received a SIGTERM\n", __FUNCTION__, __LINE__);
				exit_server(0);
			}
		}
		...
		case DHCPDISCOVER:
			...
			if (sendOffer(&packet) < 0) {
				...
				syslog(LOG_NOTICE,"send OFFER failed");
			}
			break;			
 		case DHCPREQUEST:
			...
			if (lease) { /*ADDME: or static lease */
				if (server_id) {
					...
					//syslog(LOG_NOTICE,"server_id = %08x", ntohl(server_id_align));
