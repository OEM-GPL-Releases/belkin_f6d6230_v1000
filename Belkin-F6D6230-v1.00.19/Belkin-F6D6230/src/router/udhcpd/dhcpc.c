...
static void background(void)
{
	...
	OPEN_LOG("udhcpc");		// need openlog in child process, or no information displayed, by WindChen
	//LOG(LOG_INFO, "udhcp client (v%s) started", VERSION);
}

static void add_static_route(unsigned char *temp, int len)
{
	int i;
	int rt_num;
	unsigned char *walker;
	unsigned long *host, *gw;
	char buf1[20], buf2[20];
	char command[64];
	struct in_addr host_addr, gw_addr;

	//cprintf("[add_static_route]: Enter!! line %d\n", __LINE__);

	if((len%8) > 0)
	{
		//cprintf("[add_static_route]: Incorrect length, return, line %d\n", __LINE__);
		return;
	}
	else
		rt_num = (len/8);
	...
	for(i = 0 ; i < rt_num ; i++)
	{
		...
		sprintf(buf1, "%s", inet_ntoa(host_addr));

		//Get gateway ipaddress
		gw   = (walker + 4);
		gw_addr.s_addr = *gw;
		sprintf(buf2, "%s", inet_ntoa(gw_addr));
		
		//Compose route add command
		sprintf(command, "routeadd %s %s &", buf1, buf2);
		system(command);
	}
...
#endif
{
	...
	for (;;) {
		...
		if (retval == 0) {
			...
			case REBINDING:
				if (*message == DHCPACK) {
					temp = get_option2(&packet, DHCP_STATIC_ROUTE, &opt_len);
					if(temp != NULL)
						add_static_route(temp, opt_len);
