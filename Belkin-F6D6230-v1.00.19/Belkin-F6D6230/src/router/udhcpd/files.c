...
static int read_opt(char *line, void *arg)
{
	...
	do {
		...
		{
		  //hclin fix if string constain space, will be sparate. find end of the string
	    val = strtok(NULL, "\0");		  
		}
		else
		{
		...
		if (leases[i].yiaddr != 0) {
			if (server_config.remaining) {
                         if (lease_expired(&(leases[i])))
                         	{
                          //      if( leases[i].expires<=ntohl(j) &&server_config.lease==ntohl(j) )
				//	{
				//	lease_time=ntohl(86400);
                          //      cprintf("DHCP client expire time forever\n");
                          //      }
				//	else			
					lease_time = 0;
                         	}	
