...
static void add_padding(struct dhcpMessage *packet)
{
	int end = end_option(packet->options);
	int i, len =60-end;

	for (i = 1; i<=len; i++)
       	packet->options[end +i] = 0x00;
}
...
int send_discover(unsigned long xid, unsigned long requested)
{
	...
	return raw_packet_c(&packet, INADDR_ANY, CLIENT_PORT, INADDR_BROADCAST, 
...
int send_renew(unsigned long xid, unsigned long server, unsigned long ciaddr)
{
	...
	cprintf("%s(%d) Sending renew...\n", __FUNCTION__, __LINE__);
	if (server) 
		ret = kernel_packet_c(&packet, ciaddr, CLIENT_PORT, server, SERVER_PORT);
	else ret = raw_packet_c(&packet, INADDR_ANY, CLIENT_PORT, INADDR_BROADCAST,
				SERVER_PORT, MAC_BCAST_ADDR, client_config.ifindex);
	return ret;
}	
...
int send_release(unsigned long server, unsigned long ciaddr)
{
	...
	return kernel_packet_c(&packet, ciaddr, CLIENT_PORT, server, SERVER_PORT);
}
