	...
	if (ignored_conntrack->nat_type == NFC_IP_CONE_NAT ) {
		...
		if ( h) {
			...
			if ( !nf_ct_tuple_src_equal(&ignored_conntrack->tuplehash[ IP_CT_DIR_ORIGINAL].tuple ,  &pExistedConnection->tuplehash[ IP_CT_DIR_ORIGINAL].tuple )  ) {
				return 1;
			} else { // patched by Broadcom solution to avoid NAT CONE type issue when runs with BT
				 /* anthony, use count was added at nf_conntrack_find_get() but discard the reference here. 
					roll back the use count to avoid zombie conntrack happened. */
