...
nvram_erase(void)
{
	...
	if (!nvram_mtd) {
		printk("nvram_erase: NVRAM not found\n");
		return -ENODEV;
	}

	if (in_interrupt()) {
		printk("nvram_erase: not committing in interrupt\n");
		return -EINVAL;
	}
	...
	if ((i = erasesize - NVRAM_SPACE) > 0) {
		...
		if (ret || len != i) {
			printk("nvram_erase: read error ret = %d, len = %d/%d\n", ret, len, i);
			ret = -EIO;
			goto done;
		}
	...
	if (ret || magic_len != sizeof(header->magic)) {
		printk("nvram_erase: clear MAGIC error\n");
		ret = -EIO;
		goto done;
	}
	...
	for (; offset < nvram_mtd->size - NVRAM_SPACE + header->len;
		...
		if ((ret = nvram_mtd->erase(nvram_mtd, &erase))) {
			...
			printk("nvram_erase: erase error\n");
			goto done;
		}
	...
	if (ret || len != i) {
		printk("nvram_erase: write error\n");
		ret = -EIO;
		goto done;
	}
...
{
	if (cmd == NVRAM_CLEAR_MAGIC) {
		printk("dev_nvram_ioctl\n");
		return nvram_erase();
		}
	if (cmd != NVRAM_MAGIC)
		return -EINVAL;
